...
class CreateClaimMutation(OpenIMISMutation):
    ...
    def async_mutate(cls, user, **data):
        try:
            ...
            is_claim_code_autogenerated = data.get("autogenerate_code", None)
            mutation_log = data.pop("mutation_log", None)
            ...
            if is_claim_code_autogenerated:
                ...
                data.update({"client_mutation_label": f"Create Claim - {claim.code}"})
                # signal_mutation_module_after_mutating[cls._mutation_module].send(
                #     sender=cls, mutation_log_id=mutation_log.id, data=data, user=data["info"].context.user,
