            ...
            for service in services:
                ...
                if(serviceLinked):
                    for serviceL in serviceLinked:
                        sub_item_code = serviceL.pop('subItemCode', None)
                        if serviceL.qty_asked.is_nan() :
                            serviceL.qty_asked = 0
                        itemId = Item.objects.filter(code=sub_item_code).first()
                ...
                if(serviceserviceSet):
                    for serviceserviceS in serviceserviceSet:
                        sub_serv_code = serviceserviceS.pop('subItemCode', None)
                        if serviceserviceS.qty_asked.is_nan() :
                            serviceserviceS.qty_asked = 0
                        serviceId = Service.objects.filter(code=sub_serv_code).first()
