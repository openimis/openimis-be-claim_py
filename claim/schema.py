...
def on_claim_after_mutation(sender, **kwargs):
    # TODO run only on success
    # TODO run only if autogenerated
    if kwargs.get('mutation_class', None) == 'CreateClaimMutation':
        try:
            ...
            current_message = mutation_log.client_mutation_details
            claim = Claim.objects.get(kwargs['data'].get('uuid', None))
            # 'Auto' should be not in plain string
            current_message.replace('Auto', claim.code)
            mutation_log.client_mutation_details = current_message
            mutation_log.save()
            return []
