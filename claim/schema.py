...
from .services import handle_claim_subscription_notification
...
def on_claim_after_mutation(sender, **kwargs):
    ...
    if 'data' in kwargs and kwargs['data'].get('autogenerate'):
        try:
            ...
            mutation_log.save()
            # return [] 
            #removed return as it will be execute after return statement below
        except KeyError as e:
            logger.error("Client Mutation ID not found in claim signal after mutation, error: ", e)
    handle_claim_subscription_notification(sender, **kwargs)
    return []
