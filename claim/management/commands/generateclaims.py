...
class Command(BaseCommand):
    help = (
        "This command will generate test Claims with some optional parameters. It is intended to simulate larger"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            nargs=1,
            type=int,
            help="number of services per claim, with 10% randomness",
        )
        parser.add_argument(
            "nb_items",
            nargs=1,
            type=int,
            help="number of items per claim, with 10% randomness",
        )
        parser.add_argument(
            "--verbose",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for claim_num in range(1, nb_claims + 1):
            ...
            if verbose:
                logger.debug(
                    f"{claim_num} created claim {claim} for insuree {insuree} with code {claim.code}"
                )
            for svc_num in range(1, nb_services + 1):
                ...
                if verbose:
                    logger.debug(
                        f"{claim_num} {svc_num} Created claim service {claim_service} for service {service}"
                    )
            for item_num in range(1, nb_items + 1):
                ...
                if verbose:
                    logger.debug(
                        f"{claim_num} {item_num} Created claim item {claim_item} for item {item}"
                    )

    def get_random_insuree(self):
        if not self.insurees:
            self.insurees = Insuree.objects.filter(
    ...
    def get_random_item(self):
        if not self.items:
            self.items = Item.objects.filter(validity_to__isnull=True).values_list(
                "pk", flat=True
            )
    ...
    def get_random_claim_admin(self):
        if not self.claim_admins:
            self.claim_admins = ClaimAdmin.objects.filter(
                validity_to__isnull=True
            ).values_list("pk", flat=True)
    ...
    def get_random_hf(self):
        if not self.hfs:
            ...
            self.hfs = HealthFacility.objects.filter(
                validity_to__isnull=True
            ).values_list("pk", flat=True)
